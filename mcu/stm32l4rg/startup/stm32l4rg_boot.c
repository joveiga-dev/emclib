/*
* Very simple startup code for stm32l476rg cortex-m core 
*/
...
/**
 * Zero static variables with zero or no initial value
 */
void zero_bss(void)
{
    uint32_t *p_dst = &__start_bss;
    while (p_dst < &__end_bss)
    {
        *p_dst++ = 0;
    }
}
...
void copy_data(void)
{
    uint32_t *p_dst = &__start_data;
    uint32_t *p_src = &__load_data;
    while(p_dst < &__end_data)
    {
        *p_dst++ = *p_src++;
    }
}
...
const int_handler_t vectors[] = {
    (int_handler_t)STACK_TOP, 
    Reset_Handler,
    NMI_Handler,
    HardFault_Handler,
